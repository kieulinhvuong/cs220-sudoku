...
public class SudokuGUI extends JFrame {
	...
	private class MyKeyListener extends KeyAdapter {
		...
		// Feature 1: print if the input number is legal
		public void keyTyped1(int digit) {
			sudoku.set(row, col, digit);
			JOptionPane.showMessageDialog(null, String.format("%d is legal in row %d and col %d", digit, row, col));
		}
	...
	private class ButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			// XXX was the piece we were missing
			hintRow = -1;
			hintCol = -1;
		...
		// Feature 2: print if the posiiton is blank
		public void printIfBlank() {
			JOptionPane.showMessageDialog(null, "Enter a number here!");
		}
	}
	...
	private void update() {
		for (int row = 0; row < numRows; row++) {
			for (int col = 0; col < numCols; col++) {
				if (hintRow == row && hintCol == col && sudoku.isBlank(row, col)) {
